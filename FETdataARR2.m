function [MAT] = FETdataARR2(folder,rep,Vds,Ids_col,Vgs_col,Vgs_size)
    format short e;
    ...
    for i=1:rep
        ...
        for j=1:size(Vds,2)
            for k=1:size(Vgs,1)
            MAT(1,2*(j-1)*rep+2*i-1)  = i;
            MAT(1,2*(j-1)*rep+2*i)    = i;
            MAT(2,2*(j-1)*rep+2*i-1)  = Vds(1,j);
            MAT(2,2*(j-1)*rep+2*i)    = Vds(1,j);
            
            MAT(k+2,2*(j-1)*rep+2*i-1)= Vgs(k,1);
            MAT(k+2,2*(j-1)*rep+2*i)  = absIds(k,j);
