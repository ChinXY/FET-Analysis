function [MAT1,MAT2] = FETall_out_dataXY(folder,CHLarray,type,Vds,Ids_col,Vgs_col,Vgs_size)
    format short e;
    ...
    for i=1:size(CHLarray,2)
        for x=1:2
            if x==1
                file=['PCBM_FET_L' num2str(CHLarray(1,i)) 'W10_out_' type '1_c.csv'];
            ...
            [Vgs,Ids,~,~] = FETdataimport1(folder,file,Vds,Ids_col,Vgs_col);
            ...
            for j=1:size(Vds,2)
                ...
                MAT1(k+3,4*(i-1)*size(Vds,2)+2*(x-1)*size(Vds,2)+2*j)       = Ids(k,j);
                ...
                MAT2(k+3,4*(j-1)*size(CHLarray,2)+2*x+4*i-2-2)    = Ids(k,j);
                end
            end
        end
    end    
